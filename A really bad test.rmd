...
ifelse(2*(1-pnorm(abs(z)))<0.05,yes="Doh! No random data",no="I did not identify any dependency for this lag")
...
acf(cordat)
hist(ecdf(cordat)(cordat))
k<- ecdf(cordat)(cordat)
acf(k)
```
...
```
Inversion Method is to be used that is a flaw. That is in case of known distributions.What about distribution is unknown?
